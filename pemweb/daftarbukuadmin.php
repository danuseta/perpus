...
<head>
    ...
    <script>
        ...
        function searchBooks() {
            ...
            xhr.open("GET", "search.php?search=" + input, true);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['tambah_buku'])) {
    ...
    $target_dir = "C:/Users/danus/OneDrive/Documents/pemweb/uploads/";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ubah_buku'])) {
    ...
    $update_fields = "judul = '$judul_baru', penulis = '$penulis_baru', tahun_terbit = $tahun_terbit_baru";
...
?>
    <form action="daftarbukuadmin.php" method="get" id="searchForm">
        <label for="search"><h2>Cari Buku:</h2></label>
    ...
    if (isset($_GET['search'])) {
        $search = $_GET['search'];
        $sql_buku .= " WHERE judul LIKE '%$search%' OR penulis LIKE '%$search%'";
    }
...
$result_buku = $conn->query($sql_buku);
    ...
    if ($result_buku->num_rows > 0) {
        ...
        while ($row_buku = $result_buku->fetch_assoc()) {
            ...
            echo "<td>";
            echo "<button style='background-color: #007BFF; color: white; border: none; padding: 5px 10px; cursor: pointer;' onclick=\"showEditForm('{$id}', '{$judul}', '{$penulis}', '{$tahun_terbit}', '{$gambar}')\">Ubah</button>";
            echo "<form action='daftarbukuadmin.php' method='post' style='display:inline;'>";
    ...
    <div id="editForm" class="modal">
        <div class="modal-content">
            ...
            <h2>Ubah Buku</h2>
            <form action="daftarbukuadmin.php" method="post" enctype="multipart/form-data">
